...
def calc_rms_currents(dab_design_config):
    """
    ...
    :param dab_design_config: design configuration
    :return: i_l_s_rms, i_l_1_rms, i_l_2_rms
    """

    alpha_rad = dab_design_config.mod_zvs_phi - dab_design_config.mod_zvs_tau1
    beta_rad = np.pi + dab_design_config.mod_zvs_phi - dab_design_config.mod_zvs_tau2
    gamma_rad = np.full_like(alpha_rad, np.pi)
    delta_rad = np.pi + dab_design_config.mod_zvs_phi

    # define the full mask for mode 2, made of interval 1 and interval 2
    mode_2_mask = np.bitwise_or(dab_design_config.mod_zvs_mask_Im2, dab_design_config.mod_zvs_mask_IIm2)

    # calculate current values for l_s depend on angles. Modulation modes are taken into account
    d = dab_design_config.n * dab_design_config.mesh_V2 / dab_design_config.mesh_V1

    # currents in l_s for mode 1 and mode 2+
    m1_i_l_s_alpha_rad, m1_i_l_s_beta_rad, m1_i_l_s_gamma_rad, m1_i_l_s_delta_rad = calc_l_s_mode_1_angles_rad(dab_design_config.mod_zvs_phi, dab_design_config.mod_zvs_tau1, dab_design_config.mod_zvs_tau2, dab_design_config.mesh_V1, d, dab_design_config.fs, dab_design_config.Ls)
    m2_i_l_s_alpha_rad, m2_i_l_s_beta_rad, m2_i_l_s_gamma_rad, m2_i_l_s_delta_rad = calc_l_s_mode_2_plus_angles_rad(dab_design_config.mod_zvs_phi, dab_design_config.mod_zvs_tau1, dab_design_config.mod_zvs_tau2, d, dab_design_config.mesh_V1, dab_design_config.fs, dab_design_config.Ls)

    # currents in l_1 for mode 1 and mode 2+
    m1_i_l_1_alpha_rad, m1_i_l_1_beta_rad, m1_i_l_1_gamma_rad, m1_i_l_1_delta_rad = calc_l_1_mode_1_angles_rad(dab_design_config.mod_zvs_phi, dab_design_config.mod_zvs_tau1, dab_design_config.mod_zvs_tau2, dab_design_config.mesh_V1, dab_design_config.fs, dab_design_config.Lc1)
    m2_i_l_1_alpha_rad, m2_i_l_1_beta_rad, m2_i_l_1_gamma_rad, m2_i_l_1_delta_rad = calc_l_1_mode_2_plus_angles_rad(dab_design_config.mod_zvs_phi, dab_design_config.mod_zvs_tau1, dab_design_config.mod_zvs_tau2, dab_design_config.mesh_V1, dab_design_config.fs, dab_design_config.Lc1)

    # currents in l_2 for mode 1 and mode 2+
    m1_i_l_2_alpha_rad, m1_i_l_2_beta_rad, m1_i_l_2_gamma_rad, m1_i_l_2_delta_rad = calc_l_2_mode_1_angles_rad(dab_design_config.mod_zvs_tau2, dab_design_config.mesh_V2, dab_design_config.fs, dab_design_config.Lc2)
    m2_i_l_2_alpha_rad, m2_i_l_2_beta_rad, m2_i_l_2_gamma_rad, m2_i_l_2_delta_rad = calc_l_2_mode_2_plus_angles_rad(dab_design_config.mod_zvs_phi, dab_design_config.mod_zvs_tau1, dab_design_config.mod_zvs_tau2, dab_design_config.mesh_V2, dab_design_config.fs, dab_design_config.Lc2)
    ...
    i_l_s_alpha_rad[dab_design_config.mod_zvs_mask_IIIm1] = m1_i_l_s_alpha_rad[dab_design_config.mod_zvs_mask_IIIm1]
    i_l_s_alpha_rad[mode_2_mask] = m2_i_l_s_alpha_rad[mode_2_mask]

    i_l_s_beta_rad = np.full_like(m1_i_l_s_beta_rad, np.nan)
    i_l_s_beta_rad[dab_design_config.mod_zvs_mask_IIIm1] = m1_i_l_s_beta_rad[dab_design_config.mod_zvs_mask_IIIm1]
    i_l_s_beta_rad[mode_2_mask] = m2_i_l_s_beta_rad[mode_2_mask]

    i_l_s_gamma_rad = np.full_like(m1_i_l_s_gamma_rad, np.nan)
    i_l_s_gamma_rad[dab_design_config.mod_zvs_mask_IIIm1] = m1_i_l_s_gamma_rad[dab_design_config.mod_zvs_mask_IIIm1]
    i_l_s_gamma_rad[mode_2_mask] = m2_i_l_s_gamma_rad[mode_2_mask]

    i_l_s_delta_rad = np.full_like(m1_i_l_s_delta_rad, np.nan)
    i_l_s_delta_rad[dab_design_config.mod_zvs_mask_IIIm1] = m1_i_l_s_delta_rad[dab_design_config.mod_zvs_mask_IIIm1]
    ...
    i_l_1_alpha_rad[dab_design_config.mod_zvs_mask_IIIm1] = m1_i_l_1_alpha_rad[dab_design_config.mod_zvs_mask_IIIm1]
    i_l_1_alpha_rad[mode_2_mask] = m2_i_l_1_alpha_rad[mode_2_mask]

    i_l_1_beta_rad = np.full_like(m1_i_l_1_beta_rad, np.nan)
    i_l_1_beta_rad[dab_design_config.mod_zvs_mask_IIIm1] = m1_i_l_1_beta_rad[dab_design_config.mod_zvs_mask_IIIm1]
    i_l_1_beta_rad[mode_2_mask] = m2_i_l_1_beta_rad[mode_2_mask]

    i_l_1_gamma_rad = np.full_like(m1_i_l_1_gamma_rad, np.nan)
    i_l_1_gamma_rad[dab_design_config.mod_zvs_mask_IIIm1] = m1_i_l_1_gamma_rad[dab_design_config.mod_zvs_mask_IIIm1]
    i_l_1_gamma_rad[mode_2_mask] = m2_i_l_1_gamma_rad[mode_2_mask]

    i_l_1_delta_rad = np.full_like(m1_i_l_1_delta_rad, np.nan)
    i_l_1_delta_rad[dab_design_config.mod_zvs_mask_IIIm1] = m1_i_l_1_delta_rad[dab_design_config.mod_zvs_mask_IIIm1]
    ...
    i_l_2_alpha_rad[dab_design_config.mod_zvs_mask_IIIm1] = m1_i_l_2_alpha_rad[dab_design_config.mod_zvs_mask_IIIm1]
    i_l_2_alpha_rad[mode_2_mask] = m2_i_l_2_alpha_rad[mode_2_mask]

    i_l_2_beta_rad = np.full_like(m1_i_l_2_beta_rad, np.nan)
    i_l_2_beta_rad[dab_design_config.mod_zvs_mask_IIIm1] = m1_i_l_2_beta_rad[dab_design_config.mod_zvs_mask_IIIm1]
    i_l_2_beta_rad[mode_2_mask] = m2_i_l_2_beta_rad[mode_2_mask]

    i_l_2_gamma_rad = np.full_like(m1_i_l_2_gamma_rad, np.nan)
    i_l_2_gamma_rad[dab_design_config.mod_zvs_mask_IIIm1] = m1_i_l_2_gamma_rad[dab_design_config.mod_zvs_mask_IIIm1]
    i_l_2_gamma_rad[mode_2_mask] = m2_i_l_2_gamma_rad[mode_2_mask]

    i_l_2_delta_rad = np.full_like(m1_i_l_2_delta_rad, np.nan)
    i_l_2_delta_rad[dab_design_config.mod_zvs_mask_IIIm1] = m1_i_l_2_delta_rad[dab_design_config.mod_zvs_mask_IIIm1]
