...
from dct.circuit_enums import SamplingEnum as CircuitSampling
...
class HandleSbcDto:
    ...
    def set_c_oss_storage_directory(act_c_oss_storage_directory: str) -> bool:
        ...
        if os.path.exists(abs_dir):
            HandleSbcDto.c_oss_storage_directory = abs_dir
...
class HandleTransistorDto:
    ...
    def tdb_to_transistor_dto(transistor_name: str, c_oss_margin_factor: float = 1.2) -> d_dtos.TransistorDTO:
        ...
        if not os.path.exists(os.path.join(HandleSbcDto.c_oss_storage_directory, f"{transistor.name}_c_oss.nlc")):
            transistor.export_geckocircuits_coss(filepath=HandleSbcDto.c_oss_storage_directory,
                                                 margin_factor=c_oss_margin_factor)
