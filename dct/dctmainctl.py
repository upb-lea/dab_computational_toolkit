...
class DctMainCtl:
    ...
    def get_initialization_queue_data(act_toml_prog_flow: tc.FlowControl) \
        ...
        for entry in configuration_file_list:
            ...
            if act_toml_prog_flow.circuit.calculation_mode == CalcModeEnum.skip_mode.name:
        ...
        for entry in configuration_file_list:
            ...
            inductor_data = srv_ctl_dtos.ConfigurationDataEntryDto(
                ...
                progress_data=copy.deepcopy(progress_data_init))
            #  If optimization is skipped, set the status to 'skip'
            if act_toml_prog_flow.inductor.calculation_mode == CalcModeEnum.skip_mode.name:
        ...
        for entry in configuration_file_list:
            ...
            transformer_data = srv_ctl_dtos.ConfigurationDataEntryDto(
                ...
                progress_data=copy.deepcopy(progress_data_init))
            #  If optimization is skipped, set the status to 'skip'
            if act_toml_prog_flow.transformer.calculation_mode == CalcModeEnum.skip_mode.name:
        ...
        for entry in configuration_file_list:
            heat_sink_data = srv_ctl_dtos.ConfigurationDataEntryDto(
                ...
                progress_data=copy.deepcopy(progress_data_init))
            #  If optimization is skipped, set the status to 'skip'
            if act_toml_prog_flow.heat_sink.calculation_mode == CalcModeEnum.skip_mode.name:
    ...
    def run_optimization_from_toml_configurations(self, workspace_path: str) -> None:
        ...
        if toml_prog_flow.circuit.calculation_mode == CalcModeEnum.skip_mode.name:
        ...
        if toml_prog_flow.inductor.calculation_mode == CalcModeEnum.skip_mode.name:
                ...
                if StudyData.check_study_data(inductor_results_datapath, self._inductor_study_data.study_name):
                    self._inductor_number_filtered_analytic_points_skip_list.append(
                        self.get_number_of_pkl_files(os.path.join(inductor_results_datapath,
                                                                  "08_circuit_dtos_incl_inductor_losses")))

                    self._inductor_number_filtered_simulation_points_skip_list.append(
                        self.get_number_of_pkl_files(os.path.join(inductor_results_datapath,
                                                                  "09_circuit_dtos_incl_inductor_losses")))

                    if len(self._inductor_number_filtered_simulation_points_skip_list) < len(self._inductor_number_filtered_analytic_points_skip_list):
                        inductor_sim_calculation_mode = CalcModeEnum.continue_mode.name
        ...
        if toml_prog_flow.transformer.calculation_mode == CalcModeEnum.skip_mode.name:
            # Check if circuit is not skipped
            if toml_prog_flow.circuit.calculation_mode != CalcModeEnum.skip_mode.name:
                raise ValueError(
                    f"Circuit Study {self._circuit_optimization.circuit_study_data.study_name} is not skipped.\n",
                ...
                if StudyData.check_study_data(transformer_results_datapath, self._transformer_study_data.study_name):
                    self._transformer_number_filtered_analytic_points_skip_list.append(
                        self.get_number_of_pkl_files(os.path.join(transformer_results_datapath,
                                                                  "08_circuit_dtos_incl_transformer_losses")))

                    self._transformer_number_filtered_simulation_points_skip_list.append(
                        self.get_number_of_pkl_files(os.path.join(inductor_results_datapath,
                                                                  "09_circuit_dtos_incl_transformer_losses")))

                    if len(self._transformer_number_filtered_simulation_points_skip_list) < len(self._transformer_number_filtered_analytic_points_skip_list):
                        inductor_sim_calculation_mode = CalcModeEnum.continue_mode.name
        ...
        if toml_prog_flow.heat_sink.calculation_mode == CalcModeEnum.skip_mode.name:
            # Check, if data are available (skip case)
        ...
        if not toml_prog_flow.circuit.calculation_mode == CalcModeEnum.skip_mode.name:
            ...
            if toml_prog_flow.circuit.calculation_mode == CalcModeEnum.new_mode.name:
        ...
        # Check, if electrical optimization is not to skip
        if not toml_prog_flow.capacitor_1.calculation_mode == CalcModeEnum.skip_mode.name:
            ...
            # Check, if old study is to delete, if available
            if toml_prog_flow.capacitor_1.calculation_mode == CalcModeEnum.new_mode.name:
                # delete old circuit study data
        ...
        # Check, if electrical optimization is not to skip
        if not toml_prog_flow.capacitor_2.calculation_mode == CalcModeEnum.skip_mode.name:
            ...
            # Check, if old study is to delete, if available
            if toml_prog_flow.capacitor_2.calculation_mode == CalcModeEnum.new_mode.name:
                # delete old circuit study data
        ...
        if not toml_prog_flow.inductor.calculation_mode == CalcModeEnum.skip_mode.name:
            # Set the status to InProgress
            self._inductor_main_list[0].progress_data.progress_status = ProgressStatus.InProgress

            # Check, if old study is to delete, if available
            if toml_prog_flow.inductor.calculation_mode == CalcModeEnum.new_mode.name:
        ...
        if not toml_prog_flow.transformer.calculation_mode == CalcModeEnum.skip_mode.name:
            # Set the status to InProgress
            self._transformer_main_list[0].progress_data.progress_status = ProgressStatus.InProgress
            # Check, if old study is to delete, if available
            if toml_prog_flow.transformer.calculation_mode == CalcModeEnum.new_mode.name:
        ...
        # Check, if heat sink optimization is to skip
        if not toml_prog_flow.heat_sink.calculation_mode == CalcModeEnum.skip_mode.name:
            # Check, if old study is to delete, if available
            if toml_prog_flow.heat_sink.calculation_mode == CalcModeEnum.new_mode.name:
        ...
        if not inductor_sim_calculation_mode == CalcModeEnum.skip_mode.name:
        ...
        if not transformer_sim_calculation_mode == CalcModeEnum.skip_mode.name:
